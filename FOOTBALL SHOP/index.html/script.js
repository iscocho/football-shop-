...
function initTicketSection() {
    ...
    if (!ticketCheckbox || !ticketButton || !ticketMessage) return;

    ticketCheckbox.addEventListener('change', () => {
        if (ticketCheckbox.checked) {
            ticketButton.disabled = false;
            ticketMessage.textContent = 'You can now check World Cup ticket availability.';
        } else {
    ...
    ticketButton.addEventListener('click', () => {
        if (!ticketCheckbox.checked) return;
        const url = `https://wa.me/${phoneNumber}?text=Hello%20Football%20Shop%2C%20I%20want%20to%20check%20World%20Cup%20ticket%20availability`;
        window.open(url, '_blank');
    });
}
